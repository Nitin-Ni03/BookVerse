...
1. User clicks Google button â†’ Redirects to `http://localhost:5000/oauth2/authorization/google`
2. After Google login â†’ Redirects to `http://localhost:5173/oauth2/callback?token=...&email=...`
...
**Solution**:
1. Open browser console â†’ Application â†’ Local Storage
...
- âœ… Works alongside existing email/password authentication
- âœ… Automatically registers new users via Google
- âœ… Links Google accounts to existing users
- âœ… Generates JWT tokens for API access
- âœ… Handles role-based redirects (admin vs. user)
- âœ… Stores user profile pictures from Google
- âœ… Marks Google users as verified by default
