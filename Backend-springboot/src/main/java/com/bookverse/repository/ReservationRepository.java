...
import com.bookverse.domain.ReservationStatus;
import com.bookverse.modal.Reservation;
...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
       ...
       @Query("SELECT r FROM Reservation r WHERE r.book.id = :bookId " +
                     "AND r.status = com.bookverse.domain.ReservationStatus.PENDING ORDER BY r.reservedAt ASC")
        ...
        */
       @Query("SELECT r FROM Reservation r WHERE r.book.id = :bookId " +
                     "AND r.status = com.bookverse.domain.ReservationStatus.PENDING ORDER BY r.reservedAt ASC LIMIT 1")
       ...
       @Query("SELECT CASE WHEN COUNT(r) > 0 THEN true ELSE false END FROM Reservation r " +
                     ...
                     "AND (r.status = com.bookverse.domain.ReservationStatus.PENDING OR r.status = com.bookverse.domain.ReservationStatus.AVAILABLE)")
       ...
       @Query("SELECT COUNT(r) FROM Reservation r WHERE r.book.id = :bookId " +
                     "AND r.status = com.bookverse.domain.ReservationStatus.PENDING")
        ...
        */
       @Query("SELECT r FROM Reservation r WHERE r.status = com.bookverse.domain.ReservationStatus.AVAILABLE " +
       ...
       @Query("SELECT r FROM Reservation r WHERE " +
                     ...
                     "(:activeOnly = false OR (r.status = com.bookverse.domain.ReservationStatus.PENDING OR r.status = com.bookverse.domain.ReservationStatus.AVAILABLE))")
