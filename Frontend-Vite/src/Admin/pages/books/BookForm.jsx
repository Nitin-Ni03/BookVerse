...
const BookForm = ({
  ...
  const handleSubmit = async () => {
    ...
    // Basic validation
    if (!formData.title || !formData.author || !formData.isbn || !formData.genreId || !formData.totalCopies) {
      setSubmitError("Please fill in all required fields.");
      return;
    }

    // Prepare data for submission - sanitize types
    const submissionData = {
      ...formData,
      pages: formData.pages === "" ? null : formData.pages,
      price: formData.price === "" ? null : formData.price,
      genreId: parseInt(formData.genreId), // Ensure genreId is a number
    ...
    try {
      if (editingBook) {
        await dispatch(
          updateBook({ id: editingBook.id, bookData: submissionData })
        ).unwrap();
      } else {
        await dispatch(createBook(submissionData)).unwrap();
      }
      handleCloseDialog();
      //   loadBooks();
    } catch (error) {
      console.error("Error saving book:", error);
      setSubmitError(typeof error === 'string' ? error : "Failed to save book. Please try again.");
    }
  };
            ...
            <TextField
              ...
              onChange={(e) => {
                ...
                setFormData({
                  ...
                  availableCopies: formData.availableCopies > newTotal ? newTotal : formData.availableCopies,
                });
              }}
              required
