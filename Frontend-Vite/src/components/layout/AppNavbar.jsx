...
const AppNavbar = () => {
  ...
  return (
    <nav className="fixed w-full bg-white/95 backdrop-blur-sm shadow-sm z-50 border-b border-gray-200" style={{ backgroundColor: 'var(--color-card)', borderColor: 'var(--color-border)' }}>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  to={link.path}
                  className={`flex items-center space-x-1 px-4 py-2 rounded-lg font-medium transition-all duration-200 ${isActive
                    ? 'bg-indigo-50 text-indigo-700'
                    : 'text-gray-700 hover:bg-gray-50 hover:text-indigo-600'
                    }`}
                  style={{
                    color: isActive ? 'var(--color-primary)' : 'var(--color-textSecondary)',
                    backgroundColor: isActive ? 'var(--color-backgroundSecondary)' : 'transparent',
                  }}
                >
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-gray-200 shadow-lg" style={{ backgroundColor: 'var(--color-card)', borderColor: 'var(--color-border)' }}>
          <div className="px-4 py-4 space-y-2">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  to={link.path}
                  className={`flex items-center space-x-2 px-4 py-3 rounded-lg transition-colors ${isActive
            ...
            <div className="px-4 py-2">
              <div className="flex items-center justify-between">
                <span className="text-gray-700 font-medium" style={{ color: 'var(--color-textSecondary)' }}>Theme</span>
                <ThemeToggle />
              </div>
            </div>
            ...
            {!isAuthenticated && (
              <div className="pt-4 border-t border-gray-200 space-y-2" style={{ borderColor: 'var(--color-border)' }}>
                <Link
                  to="/login"
