# BookVerse Library - Library Management System
...
- **Email Notifications**: Automated reminders for due dates, overdue books, and available reservations
- **OAuth2 Login**: Seamless authentication integration with Google
- **Wishlist & Reviews**: Users can save books to their wishlist and leave ratings and reviews
- **Fine Management**: Automated fine calculation for overdue book returns
- **AI Integration**: AI-powered features configured via Gemini API
...
- **Spring Boot 3.5**: Core Java REST API framework
...
- **Razorpay & Stripe SDK**: Server-side payment gateway integration
- **Spring Mail**: SMTP email notification handling
- **OAuth2 Client**: Google single sign-on (SSO) integration
...
## How It Works (System Functioning)

The Library Management System operates as a decoupled full-stack web application:

1. **User Flow & Authentication**: Users can register using email/password or OAuth2 (Google). Upon successful login, the backend issues a JWT token. The frontend stores this token and attaches it to the `Authorization` header for all protected API requests.
2. **Book Discovery & Loans**: Users can browse books categorized by genres, add them to wishlists, or read reviews. When a user borrows a book, the system tracks the loan period. If a book is unavailable, users can place a reservation.
3. **Automated Tasks & Notifications**: The backend utilizes Spring Scheduling to run periodic checks for overdue books, subscription expirations, and reservation deadlines. It automatically calculates fines and triggers email notifications to users via Spring Mail.
4. **Subscription & Payments**: Premium features are gated behind subscription plans. The system integrates with Stripe and Razorpay to process payments securely. Once a payment is verified, the user's role and subscription status are dynamically updated.
